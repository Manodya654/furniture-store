...
function App() {
  ...
  // පිටුව මාරු වන විට එය Save කරයි
  useEffect(() => {
  ...
  // --- Logic Table ---
  if (currentPage === 'home') {
    return <HomePage onStartDesign={() => setCurrentPage('gallery')} onOpenGallery={() => setCurrentPage('gallery')} onLogout={() => setCurrentPage('login')} />;
  }
  ...
  if (currentPage === 'gallery') {
    return <GalleryPage onBack={() => setCurrentPage('home')} onSelectModel={() => setCurrentPage('designer')} />;
  }
    ...
    <div className="flex flex-col h-screen w-screen overflow-hidden text-white bg-[#020617] font-sans">
      <header className="h-[60px] bg-slate-900 flex items-center px-8 border-b border-slate-800 justify-between">
        ...
        <div className="flex bg-slate-800 p-1 rounded-xl">
           <button onClick={() => setViewMode('2d')} className={`px-4 py-1.5 rounded-lg text-[10px] font-bold ${viewMode === '2d' ? 'bg-emerald-600' : ''}`}>2D</button>
           <button onClick={() => setViewMode('3d')} className={`px-4 py-1.5 rounded-lg text-[10px] font-bold ${viewMode === '3d' ? 'bg-emerald-600' : ''}`}>3D</button>
        </div>
      </header>
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 relative bg-[#0a0a0a]">
          {viewMode === '3d' ? <ThreeScene onSelect={setSelected} selected={selected} roomDimensions={roomDimensions} furniture={furniture} /> : <TwoD_Scene onSelect={setSelected} selected={selected} roomDimensions={roomDimensions} furniture={furniture} />}
        </main>
